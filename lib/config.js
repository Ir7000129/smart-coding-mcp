...
const DEFAULT_CONFIG = {
  ...
  chunkSize: 25,
  chunkOverlap: 5,
  batchSize: 100, // HEAD has 100, PR#4 typically 100
  maxFileSize: 1048576,
  maxResults: 5,
  enableCache: true,
  ...
  // Embedding Configuration (Prefer HEAD as it supports Nomic/MRL)
  workerThreads: "auto",
  embeddingModel: "nomic-ai/nomic-embed-text-v1.5",
  embeddingDimension: 128, // MRL dimension
  ...
  // Resource throttling (HEAD/PR#4)
  maxCpuPercent: 25,        // Max CPU usage during indexing
  batchDelay: 250,          // Delay between batches in ms
  maxWorkers: 'auto',       // Max worker threads

  // Startup behavior
  autoIndexDelay: null,     // Delay before auto-indexing (ms)
...
export async function loadConfig(workspaceDir = null) {
  ...
  if (config.smartIndexing !== false) {
    ...
    if (detectedTypes.length > 0) {
      ...
      // Merge user patterns again to ensure they take precedence if needed
      if (userConfig.excludePatterns) {
        config.excludePatterns = [...new Set([...config.excludePatterns, ...userConfig.excludePatterns])];
      }
...
function applyEnvOverrides(config) {
  const envMap = {
    ...
    // HEAD additions
    SMART_CODING_EMBEDDING_DIMENSION: 'embeddingDimension',
    SMART_CODING_DEVICE: 'device',
    SMART_CODING_CHUNKING_MODE: 'chunkingMode',
    SMART_CODING_MAX_CPU_PERCENT: 'maxCpuPercent',
    SMART_CODING_BATCH_DELAY: 'batchDelay',
    SMART_CODING_MAX_WORKERS: 'maxWorkers'
  };
